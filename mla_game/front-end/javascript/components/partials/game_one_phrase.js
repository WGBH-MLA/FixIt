...
class Phrase extends React.Component{
  ...
  render(){
    ...
    if(currentSegment && details.user_can_vote) {
    ...
    return(
      <div>
        ...
        <button disabled={buttonDisabled ? true : false}  ref={(button) => {this.button = button}} className='text' onClick={() => this.markPhrases()} id={details.pk}>
          <span className={buttonDisabled ? '' : 'context'}>{details.text}</span> 
        </button>
      </div>
    )
  }
