...
class Phrase extends React.Component{
  ...
  render(){
    ...
    let current = active === keys || active === keys + 1 || active === keys -1
    console.log(current)
    return(
      <div>
        ...
        <button disabled={current ? false : true }ref={(button) => {this.button = button}} className='text' onClick={() => this.markPhrases()} id={details.pk}>
          <span className={ current ? 'context' : '' }>{details.text}</span> 
        </button>
      </div>
    )
  }
